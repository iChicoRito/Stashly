...
- [ ] Create the page component at `src/app/(app)/dev/storage/page.tsx`, rendering the card. **Routing and the sidebar entry are NOT part of this task** — Task 9 registers the `dev/storage` route and Task 10 adds the DEV-only sidebar entry. (An earlier draft of this line said to create the route and gate the sidebar entry here, which contradicted the other two tasks; the page component is created unrouted, so nothing renders the panel until Task 9 lands. That is expected, not a defect.)
...
- [ ] **Count code points, not UTF-16 units (Ruling 26, found in Task 4's review):** Rust's `chars().count()` counts Unicode scalar values while JavaScript's `String.prototype.length` counts UTF-16 code units, so a rule written as `password.length < 8` would **accept** `"🔒🔒🔒🔒"` — 8 UTF-16 units but only 4 characters — which `vault.rs` rejects. The user would meet a validation error at submit with no way to tell why. Every length rule in `onboarding-schema.ts` must use `[...value].length`, and a test must pin it with an astral-plane character (an emoji or a surrogate pair) as the input. The reverse case is merely annoying rather than wrong (a 120-emoji name is 240 TS units, so the wizard would reject a name Rust accepts), but it should be consistent too. Note also that JS `.trim()` and Rust `.trim()` disagree about U+FEFF and U+0085, so `toSubmission` must trim with the same intent as `validate_submission` does — trim, then measure — rather than relying on the two languages agreeing on what whitespace is.
...
**Execution note:** Task 9 owns the final `identifier` value. Task 11 adds the cross-platform targets and workflow; it must not freeze or restore the identifier. The original `com.stashly.app` value produced a macOS advisory warning because it ends in `.app`; the user approved changing it to `com.stashly.desktop` in Task 9 before any real Phase 1 vault exists. The workflow and config validation below must preserve `com.stashly.desktop`.

- [ ] Leave `identifier` at the value Task 9 set (`"com.stashly.desktop"`); preserve `productName`, `version`, the window config, `frontendDist`, `devUrl`, and `security.csp` exactly as Task 9 set them.
...
- [ ] Write `docs/verification/phase-1/00-summary.md` with: the two walkthroughs and their observed results; the **confirmed** resolved vault root and DB path from Task 5 (which must now be under the final `com.stashly.desktop` identifier); the exact `cargo`/`npm`/`biome` commands and results; and a plainly-worded limits section stating that **macOS was not built or run on this host** (Windows host, only `x86_64-pc-windows-msvc` installed, no macOS toolchain) and that macOS evidence comes from the Task 11 CI matrix.
- [ ] **Carry forward every `Ruling:` and `minor (deferred):` line from the execution ledger** (`.superpowers/sdd/2026-09-13-phase-1-vault-foundation/progress.md`) into the summary's deferrals table, so the decisions taken during execution and the minors deliberately parked are visible to the next phase rather than dying with the scratch workspace. Two are specifically required by earlier rulings: the `migrate` `version > SCHEMA_VERSION` guard the v2 author must add before the loop (Ruling 9), and `get_settings_page` being superseded as a Task 3 deliverable (Ruling 14). Include Rulings 38–39's identifier decision and state explicitly that no continuity with any pre-rename `com.stashly.app` test directory is claimed.
